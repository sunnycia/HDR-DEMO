...
class Viewer(object):
    ...
    def __init__(self, width=DEFAULT_WIDTH, height=DEFAULT_HEIGHT, depth=DEFAULT_DEPTH):
        ...
        self.box = Gtk.Box()
        self.btn = Gtk.Button('Choose file')
        self._painter = Gtk.DrawingArea()
        print(dir(self._painter.props))
        self._width = width
        self._height = height
        self._depth = depth
    ...
    def _initUI(self):
        ...
        self.box.add(self._painter)
        self._painter.set_size_request(self._width,self._height)
        self.btn.connect("clicked", self.on_file_clicked)
